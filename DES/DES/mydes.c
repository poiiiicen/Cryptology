...
static char key_rol_steps[16] =
{
	...
	28 /* total steps to rotate left before this round */
...
void des_set_key(char *key) /* 64 bits (will use only 56) */ {
	...
	*/
	...
	for (j = 0; j < 56; j++) /* convert key_perm_table to bits of key */ { /* [%] j is the target bit number of key */
	...
	for (i = 0; i < 16; i++) /* key chunk for each iteration */ {
		...
		for (j = 0; j < 48; j++) /* select bits individually */ { /* [%] Select 48 bits from 56 bits.
																  j is the target bit number, and
																  key_56bit_to_48bit_table[j]-1 is
																  the source bit number */
																  /* check bit that goes to kn[j] */
			if (pcr[key_56bit_to_48bit_table[j] -
				1]) /* [%] pcr[key_56bit_to_48bit_table[j]-1] is SourceBit, j is TargetBit */ {
...
static void permute(char *inblock, char perm[16][16][8], char *outblock) { /* result into outblock, 64 bits */
	register int i, j;
	...
	pin = inblock;
	for (j = 0;
		j < 16; j += 2, pin++) /* [%] total 8 loops to permute 64 bits from inblock */ { /* for each input nibble */
		pout = outblock;
		p = perm[j][(*pin >> 4) & 0x0F];
		q = perm[j + 1][*pin & 0x0F];
...
static long32 f(ulong32 r, unsigned char subkey[8]) {
	...
	/*
	* translate r into s[]
	* use high address to store the high bit of r
	*/
	for (int i = 0; i < 4; i++) {
		s[i] = r % 256;
		r /= 256;
	...
	/*
	* Expansion each 4-bits of s[] into 6-bits of plaintext[]
	* plaintext_32bit_expanded_to_48bit_table is the map to match the new position i to the original position plaintext_32bit_expanded_to_48bit_table[i]
	* Because the large value in plaintext_32bit_expanded_to_48bit_table means the low bit, we use 32 to minus the value
	*/
	for (int i = 0; i < 8; i++) {
		plaintext[7 - i] = 0;
		for (int j = 0; j < 6; j++) {
	...
	*/
	...
	/*
	* use sbox to find the corresponding value
	* the highest bit and the lowest bit become the row number
	* the remaining 4 bits become the column number
	* two 4-bits become a 8-bits number into m[]
	*/
	...
	/*
	* sbox_perm_table is the map to match the new position i to the original position sbox_perm_table[i]
	* Because the large value in sbox_perm_table means the low bit, we use 32 to minus the value
	*/
	for (int i = 0; i < 4; i++) {
		t[i] = 0;
	}
	...
	/*
	* translate 4 bytes in t[] to rval
	*/
	for (int i = 0; i < 4; i++) {
		rval <<= 8;
		rval |= t[3 - i];
...
static void perm_init(char perm[16][16][8], char p[64]) /* 64-bit, either init or final */ {
	...
	for (i = 0; i < 16; i++) /* [%] i=SectionNum, each section contains 4 bits; */ /* each input nibble position */ {
		for (j = 0; j < 16; j++) /* [%] j=SectionValue varying from 0 to 15; */ /* each possible input nibble */ {
			for (k = 0; k <
				64; k++) /* [%] k=TargetBit; */ /* each output bit position */ { /* [%] check TargetBit k is concerned with SectionNum i and SectionValue j */
	...
	*/
	for (p = 0; p < 32; p++) /* [%] p=SourceBit */ {
		for (i = 0; i < 32; i++) /* [%] i=TargetBit */ {
			if (sbox_perm_table[i] - 1 ==
				p) /* [%] sbox_perm_table[i] ranges within [1,32], so it is base 1, not base 0 */ {
